...
var _ = Describe("", Label(OptionalLabel, EndpointSliceLabel), func() {
	...
	Specify("Exporting a service should create an MCS EndpointSlice in the service's namespace in each cluster with the "+
		...
		for i, client := range clients {
			eps := t.awaitMCSEndpointSlice(&client, nil, reportNonConformant(fmt.Sprintf(
...
func (t *testDriver) awaitMCSEndpointSlice(c *clusterClients, verify func(Gomega, *discoveryv1.EndpointSlice), desc ...any) *discoveryv1.EndpointSlice {
